...
def search(lst, dct, sp, var):
    lc = lst.copy()
    if len(lc) == 0:
        return 'None'
    if lc[-1][1] == sp:
        if var in dct[lc[-1][0]]:
            return lc[-1][0]
        else:
            sp = lc[-1][0]
    lc.pop()
    return search(lc, dct, sp, var)
...
for i in range(n):
    com, x, y = input().split(' ')
    if com == 'add':
        if dict_sv.get(x) is None:
            dict_sv[x] = [y, ]
        else:
            dict_sv[x].append(y)
    if com == 'create':
        lst_spaces.append((y, x))
        if dict_sv.get(x) is None:
            dict_sv[x] = [None, ]
        if dict_sv.get(y) is None:
            dict_sv[y] = [None, ]
    if com == 'get':
        if y in dict_sv[x]:
            print(x)
        else:
            print(search(lst_spaces, dict_sv, x, y))
